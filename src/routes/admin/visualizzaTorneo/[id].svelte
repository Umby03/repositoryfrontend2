...
<script lang="ts">
	...
	onMount(async () => {
		const response = await fetch('http://192.168.210.55:8080/tournament/searchID', {
			method: 'post',
  ...
  let results2=[];
	...
	async function unisciti (){
		...
		if (typeof localStorage != 'undefined') {
            fetch('http://192.168.210.55:8080/tournament/', { //mettere il percorso giusto
...
async function selezionaclub( id){
	...
	if (typeof localStorage != 'undefined') {
            fetch('http://192.168.210.55:8080/tournament/searchID', {
				method: 'post',
